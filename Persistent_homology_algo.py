...
def main_dense(filtration_file_path, debug= True):
	obj = Persistent_homology()
	...
	if debug:
		...
		print(obj.bmatrix) # for ./sample.txt
	obj.reduce_bmatrix_using_gauss()
	if debug:
		print("Final matrix: ")
		print(obj.bmatrix) # for ./sample.txt
...
def construct_S_d(d=3):
	# d+1 points
	data = []
	points = list(np.arange(d+1))
	for k in range(1, d+1):
		for vert in itertools.combinations(points,k):
			data.append(Simplex(k, k, vert))

	return data
def construct_B_d(d=3):
	# d+1 points
	data = []
	points = list(np.arange(d+1))
...
def main_S_d(d = 3, version = "sparse"):
	obj = Persistent_homology()
	obj.set_data(construct_S_d(d = d))
	obj.sort_by_lexicographic()
	...
	if version == "dense":
		...
		print("'version' should be 'sparse' or 'dense'.")
		raise ValueError
	obj.save_barcode("./output/output_S_"+str(d)+".txt")

def main_B_d(d = 3, version = "sparse"):
	obj = Persistent_homology()
	obj.set_data(construct_B_d(d = d))
	obj.sort_by_lexicographic()
	obj.initialize_dict_for_sparse_matrix()
	if version == "dense":
		obj.calculate_boundary_matrix()
		...
		print("'version' should be 'sparse' or 'dense'.")
		raise ValueError
	obj.save_barcode("./output/output_B_"+str(d)+".txt")
...
if __name__ == "__main__":
	...
	############################
	# main_sparse("./sample.txt", debug = False)
	
	# main_sparse("./filtration_cloud1.txt", debug = False)
	# main_sparse("./filtrations/filtration_D.txt", debug = False)
	# main_sparse("./filtration_bbob_f001_i01_d02.txt", debug = False)
	# main_sparse("./coco_Rips_filtrations/filtration_bbob_f021_i03_d02.txt", debug = False)
	
	# main_sparse("./coco_Rips_filtrations3/filtration_bbob_f001_i02_d02.txt", save2dir ="./coco_barcodes3/", mode_COCO=True)
	...
	# draw_barcode("./coco_barcodes/filtration_bbob_f006_i01_d02_barcode.txt", H_max_dim =1)



	# draw_barcode("./output.txt")
	# draw_barcode_diagram("./output.txt")
	# draw_barcode("../TD7/barcodes/tr_reg_064_barcode.txt")
	# draw_barcode("../TD7/barcodes/tr_reg_064_barcode.txt", H_max_dim = 3)
	# draw_barcode_diagram("../TD7/barcodes/tr_reg_064_barcode.txt")

	# draw_barcode("./coco_barcodes/filtration_bbob_f007_i02_d02_barcode.txt", savefig = True)
	# draw_barcode("./filtration_bbob_f021_i03_d02_barcode.txt")
	
	# draw_barcode_diagram("./filtration_bbob_f001_i01_d02_barcode.txt", upper_bound_multiplied_by= 3, H_max_dim = 1)
	# draw_barcode_diagram("./filtration_bbob_f021_i03_d02_barcode.txt", upper_bound_multiplied_by= 3, H_max_dim = 1)




	############## Experiement for S^d and B^d ###############
	# Results:
	# B_d: dense(not optimized): 4.1 s, sparse: 25.9 s 
	# S_d: dense(not optimized): 4.26 s, sparse: 17.6 s
	# Comment: Even "dense" version is not optimized, the fact that using numpy matrix 
	# operation is definitively more efficient.
	# for i in range(1, 11):
	# 	main_B_d(d = i, version= "dense") 
	# 	main_S_d(d = i, version= "dense")   
	###########################################################
	...
	# ps.print_stats(10)
	# ps.dump_stats("profiler_analysis.txt")
